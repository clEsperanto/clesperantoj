...
public class Tier1 {
	...
	/**
	 * Applies a generalized Kuwahara filter (Kuwahara et al.
	 * , 1976) for edge-preserving denoising.
	 * In 2D the image is locally divided into 4 quadrants; in 3D into 8 octants.
	 * Within each region the mean and variance are computed with Gaussian weights.
	 * The output pixel is a weighted blend of the region means, where regions with lower variance contribute more — preserving sharp edges while smoothing homogeneous areas.
	 * Increasing @p radius enlarges the local window (kernel_size = 2 * radius + 1), which strengthens denoising but blurs fine details.
	 * Increasing @p sigma widens the Gaussian weighting within the window, giving more influence to the center and reducing the effective smoothing at the window boundary; a very small sigma makes all samples equally weighted (box-like), while a large sigma approaches the classical (unweighted) Kuwahara.
	 * @param device ({@link DeviceJ}) - Device to perform the operation on.
	 * @param input ({@link ArrayJ}) - Input image to process.
	 * @param output ({@link ArrayJ}) - Output result image. (default: None)
	 * @param radius (int) - Half-size of the local window; kernel_size = 2 * radius + 1. (default: 1)
	 * @param sigma (float) - Standard deviation of the Gaussian weighting within the window. (default: 1.0)
	 * @return {@link ArrayJ}
	 * @see <a href="https://doi.org/10.1007/978-1-4684-0769-3_13">978-1-4684-0769-3_13</a>
	 * @throws NullPointerException if any of the device or input parameters are null.
	 */
    public static ArrayJ kuwaharaFilter(DeviceJ device, ArrayJ input, ArrayJ output, int radius, float sigma) {
        Objects.requireNonNull(device, "device cannot be null");
		Objects.requireNonNull(input, "input cannot be null");
        return new ArrayJ(net.clesperanto._internals.kernelj.Tier1.kuwahara_filter(device.getRaw(), input.getRaw(), output == null ? null : output.getRaw(), radius, sigma));
    }
